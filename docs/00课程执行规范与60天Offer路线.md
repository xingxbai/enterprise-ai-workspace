...
## 取消、重试、Streaming、Tool、Agent 或写入分支（按本节实际需要）
...
- “完整运行流程”必须从用户操作开始，按照真实执行顺序覆盖本节涉及的客户端、AI BFF、身份/权限、请求校验、业务数据、AI 应用服务、Prompt/上下文、Provider、输出解析、运行时校验、审计和最终页面状态，不能只贴一段脱离代码的架构图。
- 流程后必须提供逐步表格，至少写清每一步的输入、处理、输出或失败；存在 Streaming、取消、重试、Tool、Agent 或真实写入时，必须补充相应分支流程和失败终点。
- “本节需要特别注意的点”必须集中整理本节实际涉及的业务边界、安全边界、数据最小化、标签泄漏、Provider 兼容性、错误处理、人工确认、日志脱敏和评测要求，方便学习者课后一次性复盘。
- 每章必须提供“运行时每层能看到什么”表格，区分浏览器 Network、AI BFF、应用服务、Provider Adapter、日志/审计和业务 UI 的可见字段与禁止暴露字段。
- 涉及预测、计算或决策时，必须用表格区分确定性代码与 AI 的职责。时间、金额、权限、精确统计和可验证规则优先由普通代码计算；AI 只负责需要语义理解的分类、解释、建议或不确定性判断。
- 真实结果一般、发生错误或与确定性事实冲突时，必须保留结果、指标和定位过程，不能只展示成功案例；文档要明确问题属于数据、规则、Prompt、Provider、解析还是评测层，以及在哪一节继续修正。
