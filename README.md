...
- 已完成课程：Day01～Day12。
- 当前系统：客服旧系统基线已完成，并在工单列表接入第一项 AI 能力。
- 当前课程：Day12，工单分类、优先级和 SLA 风险结构化预测。
- 下一节课：Day13，沿 BFF、Provider、模型响应、完成原因和日志调试完整 AI 调用链。
- 当前数据：工单优先读取真实业务 API；开发环境不可用时使用明确标注的 demo fixture。其他业务模块当前使用非生产 demo fixture，生产环境不会返回演示数据。
- 当前 AI 状态：工单列表可按单条工单生成结构化分诊建议；浏览器只提交 `ticketId`，结果不会自动覆盖人工字段。历史回复建议源码继续保留，后续迁移到工单详情页。
...
- [第 12 天：工单分类、优先级与 SLA 风险结构化预测](./docs/Day12工单分类优先级与SLA风险结构化预测.md)
...
- 从 Day12 开始，每节课文档都固定整理完整运行流程、逐步输入/处理/输出、异常或取消分支、集中注意事项、确定性代码与 AI 的职责边界，以及运行时各层可见和禁止暴露的信息。
